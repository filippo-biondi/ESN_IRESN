...
class IIRESN(tf.keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=None, **kwargs):
        ...
        # a = [tf.stack(el, axis=0) for el in recurrent_kernels]
        # matrix = tf.stack(a, axis=0)
        # return matrix
        matrix = join_matrices(recurrent_kernels)
...
class IIRESNvsr(IIRESN):
    ...
    def __init__(self, sub_reservoirs, partitions, reservoirs_connectivity, spectral_radius, off_diagonal_limits,
        ...
        super().__init__(sub_reservoirs, reservoirs_connectivity, spectral_radius, off_diagonal_limits, gsr,
                         initializer)
        self.partitions = check_partition(partitions, sub_reservoirs)


class TrainableParameterInit(tf.keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=None, **kwargs):
        if shape[0] % len(self.vals) != 0:
            raise ValueError("tensor shape is not compatible with python list provided")
        values = self.vals * int(shape[0] / len(self.vals))
